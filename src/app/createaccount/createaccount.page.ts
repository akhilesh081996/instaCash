...
import { NavController, ActionSheetController, Platform, AlertController, IonRouterOutlet} from '@ionic/angular';
...
export class CreateaccountPage implements OnInit {
  ...
  GetLocation() {
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.maaap.setCenter(new google.maps.LatLng(this.latitude, this.longitude));
    }).catch((error) => {
      this.maaap.setCenter(new google.maps.LatLng(this.latitude, this.longitude));
    });
  }
  ...
  submit(formdata){
    ...
    this.userService.sendData('create_profile',formdata).subscribe(data => {
      if (data['status'] = 'ok') {
    this.userService.dismissLoading()
