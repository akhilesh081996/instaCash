...
export class EditprofilePage implements OnInit {
  ...
  GetLocation() {
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.maap.setCenter(new google.maps.LatLng(this.latitude, this.longitude));
    }).catch((error) => {
      this.maap.setCenter(new google.maps.LatLng(this.latitude, this.longitude));
    });
  }
  async initMap() {
    this.maap = new google.maps.Map(document.getElementById('maap'), {
      zoom: 12,
    ...
    this.directionsDisplay.setMap(this.maap);
    google.maps.event.addListener(this.maap, 'idle', ()=> {
      this.latitude=this.maap.getBounds().getCenter().lat();
      this.longitude=this.maap.getBounds().getCenter().lng();
    });
    this.addMarker();
  }
  addMarker(){
    ...
    google.maps.event.addListener(marker, 'click', () => {
      ...
      infoWindow.open(this.maap, marker);
      })
  ...
  getProfile(){
    this.userService.getData('getProfile?token='+this.user.token).subscribe(res =>{
      if(res){
        ...
        this.image_data = this.userprofile.userImage
        this.editprofile.patchValue({
          first_name:this.userprofile.first_name,
          last_name:this.userprofile.last_name,
          phone:this.userprofile.phone,
          street1:this.userprofile.street1,
          street2:this.userprofile.street2,
  ...
  copyFileToLocalDir(namePath, currentName, newFileName) {
    var file = new File()
    file.copyFile(namePath, currentName, file.dataDirectory, newFileName).then(success => {
  ...
  UploadImage(user) {
    if (this.images.length > 0) {
      ...
      fileTransfer.upload(this.images[0].filePath,'https://instapass.betaplanets.com/wp-json/mobileapi/v1/createpicture',options)
        .then((data1) => {
        }, (err) => {
          this.userService.dismissLoading();
        });
  ...
  submit(formdata){
    ...
    this.userService.sendData('updateprofile',formdata).subscribe(data => {
      if (data['status'] = 'ok') {
    this.userService.dismissLoading()
