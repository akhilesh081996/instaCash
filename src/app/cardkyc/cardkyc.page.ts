...
export class CardkycPage implements OnInit {
  ...
  ngOnInit() {
    this.kycdetail = new FormGroup({
      ...
      'business_website': new FormControl('https://instacash.com'),
  ...
  UploadImage() {
    if (this.images) {
      ...
      fileTransfer.upload(this.images[0].filePath,'https://instapass.betaplanets.com/wp-json/mobileapi/v1/addKycImage',options)
        .then((data1) => {
        }, (err) => {
      ...
      if (this.plt.is('android') && sourceType === this.camera.PictureSourceType.PHOTOLIBRARY) {
        this.filePath.resolveNativePath(imagePath)
          .then(filePath => {
            ...
            let smext = currentName.split('.').pop();
            let ext = smext.toLowerCase();
            this.copyFileToLocalDir1(correctPath, currentName, this.createFileName1(ext));
          });
      } else {
        var currentName = imagePath.substr(imagePath.lastIndexOf('/') + 1);
        var correctPath = imagePath.substr(0, imagePath.lastIndexOf('/') + 1);
  ...
  UploadImage1() {
    if (this.images1) {
      ...
      fileTransfer.upload(this.images1[0].filePath,'https://instapass.betaplanets.com/wp-json/mobileapi/v1/addKycImage',options)
        .then((data1) => {
        }, (err) => {
          this.userService.dismissLoading();
        });
  ...
  submit(data){
    ...
    this.stripe.createBankAccountToken(bankAccount).then(res =>{
      ...
      this.userService.sendData('verify_kyc_callback',data).subscribe(res =>{
        if(res['status'] == 'ok'){
          this.userService.dismissLoading()
